...
public interface ManualLendingRepository extends JpaRepository<ManualLending,Long> { Optional<ManualLending> findByUserIdAndSchemeIdIgnoreCase(Long userId,String schemeId); List<ManualLending> findByUserIdOrderByInvestmentDateDesc(Long userId);
 ...
 @Query("select count(m) from ManualLending m where m.userId=:u and m.npaAmount>0") long countNpa(@Param("u") Long userId);
 @Query("select coalesce(sum(m.npaAmount),0) from ManualLending m where m.userId=:u and m.npaAmount>0") BigDecimal totalNpaAmount(@Param("u") Long userId); }
